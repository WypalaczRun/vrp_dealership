...
function src.VIP()
	...
	if user_id then
		if vRP.hasPermission(user_id, "lojavip.permissao") or user_id == 357 or user_id == 146 or user_id == 1 or user_id == 144 then
			return exclusive
		else
			TriggerClientEvent('dealership:CloseTablet',source)
			TriggerClientEvent('Notify', source, 'importante', "Você não pode acessar esta categoria!")
		end
	end
end
...
function src.buyDealer(name)
	...
	if user_id then
		...
		if user_id == 146 or user_id == 144 or user_id == 1 then
			if parseInt(maxvehs[1].qtd) >= parseInt(maxgars[1].garagem) + 5000000 then
		...
		elseif vRP.hasPermission(user_id,"diamante.permissao") then
			if parseInt(maxvehs[1].qtd) >= parseInt(maxgars[1].garagem) + 9 then
				TriggerClientEvent("Notify",source,"importante","Atingiu o número máximo de veículos em sua garagem.",8000)
				return
			end
		elseif vRP.hasPermission(user_id,"phoenix.permissao") then
			if parseInt(maxvehs[1].qtd) >= parseInt(maxgars[1].garagem) + 13 then
				TriggerClientEvent("Notify",source,"importante","Atingiu o número máximo de veículos em sua garagem.",8000)
				return
			end
		else
		...
		if vehicle[1] then
			...
			if vRP.tryFullPayment(user_id,vRP.vehiclePrice(name)) then
				...
				TriggerClientEvent("Notify",source,"sucesso","Você comprou um <b>"..vRP.vehicleName(name).."</b> por <b>R$ "..vRP.format(parseInt(vRP.vehiclePrice(name))).." reais</b>.",10000)
